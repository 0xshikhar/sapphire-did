...
export class AIController {
   ...
   */
  async enhanceDescription(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  async extractTags(req: Request, res: Response): Promise<void> {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
        res.status(400).json({ error: 'Invalid input', details: errors.array() });
  ...
  async suggestCategories(req: Request, res: Response): Promise<void> {
    try {
      ...
      const categories = await this.aiService.suggestCulturalCategories(
        tags || [],
        metadata || {}
      );
  ...
  async analyzeFile(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (file.mimetype.startsWith('text/')) {
        ...
        enhancedDescription = await this.aiService.generateEnhancedDescription(
          file.originalname,
